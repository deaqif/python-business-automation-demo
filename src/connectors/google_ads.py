...
class GoogleAdsConnector:
    ...
    Demonstrates ingestion and normalization of Google Search, Display, and Performance Max metrics.
    """

    def __init__(self, sample_file: Path = None):
        self.sample_file = sample_file or (SAMPLE_DIR / "google_ads_data.json")

    def fetch_daily_metrics(self) -> Dict[str, Any]:
        """
        Fetches daily aggregated and campaign-level metrics for Google Ads.
        Returns normalized dictionary with campaign details and summary totals.
        """
        if not self.sample_file.exists():
            raise FileNotFoundError(f"Google Ads sample data not found at: {self.sample_file}")
